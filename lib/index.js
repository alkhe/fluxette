...
var _default = (function () {
	function _default() {
		...
		this.history = [];
		// React caching
		this.snapshot = (0, _util.bootstrap)(stores);
	}
	...
	_createClass(_default, [{
		...
		value: function dispatch() {
			...
			if (data.length > 0) {
				...
				(0, _util.updateState)(stores, snapshot, data);
				// Call all registered listeners
				(0, _util.callAll)(hooks, (0, _util.deriveState)(stores));
			}
		}
	}, {
		key: 'hook',
		...
		value: function connect() {
			...
			var storefn = specifier(stores);
			storefn = storefn instanceof Function ? storefn : function () {
				return storefn;
			};
			return function (Component) {
				return (function (_Component) {
					...
					function _class() {
						...
						var listener = this[_util.listenerKey] = function () {
							if (storefn[_util.changedKey]) {
								console.log(storefn);
								_get(Object.getPrototypeOf(_class.prototype), 'setState', _this).call(_this, storefn());
							}
						};
						// Register setState
						hooks.push(listener);
